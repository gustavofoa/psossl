...
public class Particle extends Thread {
	...
	public void run() {
		...
		mpca.worstSolution = mpca.bestSolution = oldSolution = mpca.generateSolution();
		
		for (int i = 0; i < MPCA.MAX; i++) {
			perturbation();
			...
			mpca.updateMessage(String
					.format("Iteração %d de %d Partícula=%d <-> Melhor solução %.3f {Centro de Massa = %.3f (x = %.2f, y = %.2f), Momento de Inércia = %.2f Kg.m2}",
							i, MAX, this.numero, mpca.bestSolution.getFitness(), mpca.bestSolution.getMassCenter(),
							mpca.bestSolution.getMassCenterX() - mpca.bestSolution.getContainer().getWidth() / 2, mpca.bestSolution.getMassCenterY()
									- mpca.bestSolution.getContainer().getHeight() / 2, mpca.bestSolution.getMomentOfInertia()));
		}
	}
	...
	private void scattering() {
		...
		if (pScattering > OptimizationAlgorithm.RANDOM.nextDouble())
			oldSolution = mpca.generateSolution();
		else
			exploration();
	}
